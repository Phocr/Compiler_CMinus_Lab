...
%token <node> COMMA
%token <node> L_PARENTHESE
%token <node> R_PARENTHESE
%token <node> L_SBRACKET
%token <node> R_SBRACKET
%token <node> L_CBRACKET
%token <node> R_CBRACKET
%token <node> ELSE
%token <node> IF
%token <node> INT
%token <node> RETURN
...
var-declaration: type-specifier ID SEMICOLON {$$=node("var-declaration", 3, $1, $2, $3);}
                | type-specifier ID L_SBRACKET NUM R_SBRACKET SEMICOLON {$$=node("var-declaration", 6, $1, $2, $3, $4, $5, $6);}
                ;
type-specifier: INT {$$=node("type-specifier", 1, $1);}
                | FLOAT {$$=node("type-specifier", 1, $1);}
                | VOID {$$=node("type-specifier", 1, $1);}
                ;
fun-declaration: type-specifier ID L_PARENTHESE params R_PARENTHESE compound-stmt {$$=node("fun-declaration", 6, $1, $2, $3, $4, $5, $6);}
                ;
...
param: type-specifier ID {$$=node("param", 2, $1, $2);}
    | type-specifier ID L_SBRACKET R_SBRACKET {$$=node("param", 4, $1, $2, $3, $4);}
    ;
compound-stmt: L_CBRACKET local-declarations statement-list R_CBRACKET {$$=node("compound-stmt", 4, $1, $2, $3, $4);}
            ;
...
expression-stmt: expression SEMICOLON {$$=node("expression-stmt", 2, $1, $2);}
                ...
                ;
selection-stmt: IF L_PARENTHESE expression R_PARENTHESE statement {$$=node("selection-stmt", 5, $1, $2, $3, $4, $5);}
            | IF L_PARENTHESE expression R_PARENTHESE statement ELSE statement {$$=node("selection-stmt", 7, $1, $2, $3, $4, $5, $6, $7);}
            ;
iteration-stmt: WHILE L_PARENTHESE expression R_PARENTHESE statement {$$=node("iteration-stmt", 5, $1, $2, $3, $4, $5);}
            ;
...
expression: var ASSIGN expression {$$=node("expression", 3, $1, $2, $3);}
            ...
            ;
var: ID {$$=node("var", 1, $1);}
    | ID L_SBRACKET expression R_SBRACKET {$$=node("var", 4, $1, $2, $3, $4);}
    ;
...
mulop: MUL {$$=node("mulop", 1, $1);}
    | DIV {$$=node("mulop", 1, $1);}
    ;
factor: L_PARENTHESE expression R_PARENTHESE {$$=node("factor", 3, $1, $2, $3);}
    | var {$$=node("factor", 1, $1);}
    | call {$$=node("factor", 1, $1);}
    | integer {$$=node("factor", 1, $1);}
    | float {$$=node("factor", 1, $1);}
    ;
integer: NUM {$$=node("integer", 1, $1);}
        ;
float: FLOATNUM {$$=node("float", 1, $1);}
    ;
call: ID L_PARENTHESE args R_PARENTHESE {$$=node("call", 4, $1, $2, $3, $4);}
    ;
